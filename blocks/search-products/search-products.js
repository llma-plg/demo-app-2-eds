...
export default async function decorate(block, bridge) {
  ...
  renderProducts(block, products, theme, bridge);
...
function renderProducts(block, products, theme, bridge) {
  ...
  scrollBtn.className = 'scroll-btn scroll-left';
  scrollBtn.setAttribute('aria-label', 'Scroll left');
  ...
  scrollBtnRight.className = 'scroll-btn scroll-right';
  scrollBtnRight.setAttribute('aria-label', 'Scroll right');
  scrollBtnRight.textContent = '▶';
  wrapper.appendChild(scrollBtnRight);
  ...
  const updateScrollButtons = () => {
    ...
    scrollBtn.style.display = atStart ? 'none' : 'flex';
    scrollBtnRight.style.display = atEnd ? 'none' : 'flex';
  ...
  scrollBtn.addEventListener('click', () => scrollByCard(-1));
  scrollBtnRight.addEventListener('click', () => scrollByCard(1));
  ...
  scrollBtnRight.addEventListener('keydown', (e) => {
    if (e.key === 'Enter' || e.key === ' ') {
      e.preventDefault();
      scrollByCard(1);
    }
